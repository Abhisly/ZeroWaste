...
import { ShieldCheck, CheckCircle, XCircle, FileText, Info, Building2, MapPin, Phone, Eye } from 'lucide-react';
...
export default function VerificationAgentDashboard() {
 const [verifications, setVerifications] = useState<any[]>(MOCK_VERIFICATIONS);
 ...
 <div>
 <h1 className="text-4xl font-display font-semibold tracking-tight text-white mb-2">Verification Queue</h1>
 ...
 <p className="text-xs font-bold text-gray-500 uppercase tracking-wide mb-1">Pending</p>
 <p className="text-2xl font-semibold text-blue-400">{verifications.filter(v => v.status === 'Pending').length}</p>
 </div>
 <div className="bg-white/5 border border-white/[0.03] p-4 rounded-xl backdrop-blur-md">
 <p className="text-xs font-bold text-gray-500 uppercase tracking-wide mb-1">Approved</p>
 <p className="text-2xl font-semibold text-green-400">{verifications.filter(v => v.status === 'Approved').length}</p>
 </div>
 </div>
 </div>
 ...
 : "bg-white/5 border-white/[0.03] hover:border-white/20"
 )}
 >
 <div className="flex justify-between items-start mb-4">
 <div className={cn(
 "px-2 py-1 rounded text-[10px] font-semibold uppercase tracking-tighter",
 ...
 className="bg-[#111111] border border-white/[0.03] rounded-3xl overflow-hidden shadow-2xl sticky top-24"
 >
 <div className="p-8 border-b border-white/[0.03] flex justify-between items-center bg-white/5">
 ...
 <div>
 <p className="text-[10px] font-semibold text-gray-500 uppercase tracking-wide mb-2">Organization Details</p>
 ...
 <div>
 <p className="text-[10px] font-semibold text-gray-500 uppercase tracking-wide mb-2">Uploaded Documents</p>
 <div className="grid grid-cols-1 gap-2">
 {selectedItem.documents.map((doc: string, idx: number) => (
 <div key={idx} onClick={() => setPreviewDoc(doc)} className="flex items-center justify-between p-3 rounded-xl bg-white/5 border border-white/[0.03] hover:border-white/20 transition-all group cursor-pointer">
 ...
 <div className="bg-white/5 rounded-2xl p-6 border border-white/[0.03]">
 <h4 className="font-bold text-white mb-4 flex items-center gap-2">
 <Info className="w-4 h-4 text-blue-400" />
 Reviewer Notes
 </h4>
 <textarea
 className="w-full h-32 bg-black/40 border border-white/[0.03] rounded-xl p-4 text-gray-300 focus:outline-none focus:border-blue-500/50 transition-all text-sm resize-none"
 placeholder="Add notes about this verification..."
 ></textarea>
 <button className="w-full mt-4 py-3 rounded-xl border border-white/[0.03] text-white/60 font-bold hover:bg-white/5 transition-all text-xs uppercase tracking-wide">
 ...
 <div className="h-[500px] border-2 border-dashed border-white/[0.03] rounded-3xl flex flex-col items-center justify-center text-center p-12">
