...
const MOCK_USERS = [
  { id: 'u1', name: 'Amul Restaurant', role: 'Restaurant', email: 'contact@amul.com', status: 'Active', joined: '2026-01-15', donations: 42 },
  { id: 'u2', name: 'Mission Hub NGO', role: 'NGO', email: 'missions@missionhub.org', status: 'Active', joined: '2026-02-01', donations: 0 },
  { id: 'u3', name: 'City Shelter', role: 'NGO', email: 'help@cityshelter.org', status: 'Active', joined: '2026-02-14', donations: 0 },
  { id: 'u4', name: 'Green Bowl', role: 'Restaurant', email: 'info@greenbowl.com', status: 'Suspended', joined: '2026-01-20', donations: 18 },
  { id: 'u5', name: 'Alex Driver', role: 'Agent', email: 'alex.d@zerowaste.in', status: 'Active', joined: '2026-03-01', donations: 0 },
  { id: 'u6', name: 'Spice Garden', role: 'Restaurant', email: 'sg@spicegarden.com', status: 'Active', joined: '2026-02-28', donations: 27 },
  { id: 'u7', name: 'Youth Hub', role: 'NGO', email: 'youth@yhub.org', status: 'Pending', joined: '2026-03-10', donations: 0 },
];

const MOCK_VERIFICATIONS = [
  { id: 'v1', name: 'Gourmet Garden', type: 'Restaurant', address: '123 Culinary Ave, Foodville', license: 'REST-5521-X', contact: '+1 555-0101', status: 'Pending', documents: ['fssai_license.pdf', 'identity_proof.jpg'], submitted: '2026-03-15' },
  { id: 'v2', name: 'Helping Hands NGO', type: 'NGO', address: '456 Community Rd, Hope City', license: 'NGO-8890-Z', contact: '+1 555-0202', status: 'Pending', documents: ['80g_certificate.pdf', 'trust_deed.pdf'], submitted: '2026-03-16' },
  { id: 'v3', name: 'FastDeliver Co.', type: 'Agent', address: '78 Logistics Park', license: 'AGENT-3367-A', contact: '+1 555-0303', status: 'Approved', documents: ['id_proof.pdf', 'vehicle_rc.pdf'], submitted: '2026-03-10' },
];
...
const INITIAL_LOGS = [
  generateLog('INFO', 'System boot complete. All services operational.', '00:01:12'),
  generateLog('INFO', 'Database cluster SYNC — 428 nodes active.', '00:12:44'),
  generateLog('WARN', 'Edge node BLR-04 latency spike: 220ms', '04:33:09'),
  generateLog('INFO', 'API Gateway health check passed.', '08:00:00'),
  generateLog('INFO', 'New user registration: Amul Restaurant (REST)', '08:14:22'),
  generateLog('ERROR', 'Auth service token refresh failed — retrying...', '09:05:17'),
  generateLog('INFO', 'Auth service recovered. Token refresh success.', '09:05:22'),
  generateLog('WARN', 'Donation pickup timeout for item ID #1023.', '11:30:45'),
  generateLog('INFO', 'Donation #1024 claimed by Mission Hub NGO.', '12:01:33'),
  generateLog('INFO', 'System snapshot saved to backup cluster.', '12:00:00'),
  generateLog('INFO', 'New NGO registration pending verification: Helping Hands', '14:22:10'),
  generateLog('WARN', 'High traffic detected on /api/donations endpoint.', '16:04:55'),
  generateLog('INFO', 'CDN cache purged and refreshed.', '18:00:00'),
  generateLog('INFO', 'Scheduled report generated and emailed to admin.', '20:00:01'),
];
...
export default function AdminDashboard() {
  ...
  const renderOverview = () => (
    <div className="space-y-10 relative">
      <div className="absolute inset-0 z-0 opacity-10 pointer-events-none overflow-hidden">
        <img src="/assets/admin_bg.png" className="w-full h-full object-cover blur-sm scale-110" alt="" />
      </div>
          ...
          return (
            <motion.div key={i} initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: i * 0.1 }} whileHover={{ y: -5, scale: 1.02 }} className="glass-dark p-6 rounded-[32px] border-white/5 flex flex-col gap-4 group cursor-default shadow-2xl backdrop-blur-3xl">
              <div className="flex justify-between items-start">
                ...
                <div className={cn("flex items-center gap-1 text-[10px] font-black px-2 py-1 rounded-lg shadow-xl", stat.isUp ? 'bg-emerald-500/10 text-emerald-400' : 'bg-red-500/10 text-red-400')}>
              ...
              <div>
                ...
                <p className="text-[10px] font-bold text-white/10 mt-1 italic">Real-time telemetry active</p>
              </div>
            </motion.div>
          );
        })}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 relative z-10">
        <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.4 }} className="lg:col-span-2 glass-dark rounded-[40px] p-8 border-white/5 overflow-hidden shadow-2xl backdrop-blur-3xl">
          <div className="flex justify-between items-center mb-10">
            <div>
              <h3 className="text-2xl font-display font-black tracking-tight italic flex items-center gap-3 underline decoration-green-500/30 underline-offset-8">NETWORK LOAD <div className="w-2 h-2 bg-green-500 rounded-full animate-ping" /></h3>
              <p className="text-sm font-bold text-white/30 uppercase tracking-[0.3em] mt-3">Global Traffic Analysis Matrix</p>
            </div>
            <div className="flex gap-2 p-1 bg-white/5 rounded-2xl border border-white/5">
              {['1H', '1D', '1W'].map(t => (
                <button key={t} className={cn("px-6 py-2.5 rounded-xl text-[10px] font-black transition-all uppercase tracking-widest", t === '1D' ? 'bg-green-500 text-white shadow-xl' : 'text-white/30 hover:text-white hover:bg-white/5')}>{t}</button>
              ))}
            </div>
          </div>
        ...
        <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.5 }} className="glass-dark rounded-[40px] p-8 border-white/5 flex flex-col shadow-2xl backdrop-blur-3xl">
          <div className="flex justify-between items-center mb-10">
            <h3 className="text-2xl font-display font-black tracking-tight italic uppercase">System Health</h3>
            <span className="text-[10px] font-black py-1 px-3 bg-green-500/10 text-green-400 border border-green-500/20 rounded-full shadow-lg">OPERATIONAL</span>
          </div>
            ...
            {[
              ...
              <div key={i} className="space-y-4">
                <div className="flex justify-between items-end">
                  <p className="font-black text-sm tracking-tight uppercase italic">{item.label}</p>
                  <span className="text-[10px] font-black text-white/20 uppercase tracking-widest">{item.status} ({item.value}%)</span>
                </div>
          ...
          <button onClick={() => setActiveTab('System Logs')} className="w-full py-6 mt-12 rounded-[28px] bg-white/5 border border-white/10 text-[11px] font-black tracking-[0.4em] hover:bg-white/10 hover:border-white/20 transition-all uppercase shadow-2xl active:scale-95">VIEW SYSTEM LOGS</button>
        </motion.div>
      </div>
    </div>
  );
    ...
    return (
      <div className="space-y-8 relative z-10">
        <div className="flex flex-col md:flex-row justify-between items-start gap-4">
          <div>
            <h1 className="text-4xl font-display font-black tracking-tighter italic uppercase">User Management</h1>
          ...
          <div className="flex gap-3 flex-wrap">
            {['All', 'Restaurant', 'NGO', 'Agent'].map(r => (
              <button key={r} onClick={() => setUserRoleFilter(r)} className={cn("px-4 py-2 rounded-xl text-xs font-black tracking-widest transition-all", userRoleFilter === r ? 'bg-green-500 text-white shadow-lg' : 'bg-white/5 text-white/30 hover:text-white hover:bg-white/10')}>{r}</button>
            ))}
          </div>
        </div>
        <div className="relative">
          <Search className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-white/20" />
          <input type="text" placeholder="Search users by name or email..." value={userSearch} onChange={e => setUserSearch(e.target.value)} className="w-full bg-white/2 border border-white/5 rounded-2xl py-4 pl-12 pr-8 text-sm font-black focus:outline-none focus:ring-2 focus:ring-green-500/20 transition-all placeholder:text-white/10" />
        </div>
        <div className="glass-dark rounded-[40px] border border-white/5 overflow-hidden shadow-2xl">
          <div className="grid grid-cols-[2fr_1fr_2fr_1fr_1fr_auto] gap-4 px-8 py-5 border-b border-white/5 text-[10px] font-black text-white/20 uppercase tracking-widest">
          ...
          <div className="divide-y divide-white/5">
            {filtered.map((user, i) => (
              <motion.div key={user.id} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.04 }} className="grid grid-cols-[2fr_1fr_2fr_1fr_1fr_auto] gap-4 px-8 py-5 hover:bg-white/2 transition-colors items-center">
                <div className="flex items-center gap-3">
                  <div className="w-9 h-9 rounded-xl bg-white/5 flex items-center justify-center shrink-0 font-black text-sm text-white/40">{user.name[0]}</div>
                  <p className="font-black text-sm truncate">{user.name}</p>
                </div>
                <span className={cn("px-2 py-1 rounded-lg text-[9px] font-black tracking-widest w-fit", user.role === 'Restaurant' ? 'bg-orange-500/10 text-orange-400' : user.role === 'NGO' ? 'bg-purple-500/10 text-purple-400' : 'bg-blue-500/10 text-blue-400')}>{user.role}</span>
                ...
                <span className={cn("px-2 py-1 rounded-lg text-[9px] font-black tracking-widest w-fit", user.status === 'Active' ? 'bg-emerald-500/10 text-emerald-400' : user.status === 'Suspended' ? 'bg-red-500/10 text-red-400' : 'bg-yellow-500/10 text-yellow-400')}>{user.status}</span>
            ...
            {filtered.length === 0 && <div className="flex items-center justify-center py-12 text-white/20"><p className="font-black text-sm uppercase tracking-widest">No users match your filter</p></div>}
          </div>
        </div>
      </div>
    );
  ...
  const renderVerifications = () => {
    ...
    return (
      <div className="space-y-8 relative z-10">
        <div className="flex justify-between items-end">
          <div>
            <h1 className="text-4xl font-display font-black tracking-tighter italic uppercase">Verification Queue</h1>
          ...
          <div className="flex gap-4">
            <div className="glass-dark border border-white/10 p-4 rounded-2xl"><p className="text-[10px] font-black text-white/30 uppercase tracking-widest mb-1">Pending</p><p className="text-2xl font-black text-yellow-400">{verifications.filter(v => v.status === 'Pending').length}</p></div>
            <div className="glass-dark border border-white/10 p-4 rounded-2xl"><p className="text-[10px] font-black text-white/30 uppercase tracking-widest mb-1">Approved</p><p className="text-2xl font-black text-emerald-400">{verifications.filter(v => v.status === 'Approved').length}</p></div>
          </div>
        </div>
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          <div className="lg:col-span-1 space-y-4">
            {verifications.map(v => (
              <button key={v.id} onClick={() => setSelectedVerif(selectedVerif?.id === v.id ? null : v)} className={cn("w-full p-6 rounded-[28px] border transition-all text-left group", selectedVerif?.id === v.id ? 'bg-green-500/10 border-green-500/40' : 'glass-dark border-white/10 hover:border-white/20')}>
                <div className="flex justify-between items-start mb-4">
                  <span className={cn("px-2 py-1 rounded-lg text-[9px] font-black uppercase tracking-tighter", v.type === 'Restaurant' ? 'bg-orange-500/20 text-orange-400' : v.type === 'NGO' ? 'bg-purple-500/20 text-purple-400' : 'bg-blue-500/20 text-blue-400')}>{v.type}</span>
                  <span className={cn("text-[10px] font-black", v.status === 'Pending' ? 'text-yellow-500' : v.status === 'Approved' ? 'text-emerald-400' : 'text-red-400')}>{v.status.toUpperCase()}</span>
                </div>
                <h3 className="text-lg font-black text-white group-hover:text-green-400 transition-colors">{v.name}</h3>
          ...
          <div className="lg:col-span-2">
            {selectedVerif ? (
              <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="glass-dark border border-white/10 rounded-[40px] overflow-hidden shadow-2xl sticky top-4">
                <div className="p-8 border-b border-white/10 flex justify-between items-center bg-white/3">
                  <div className="flex items-center gap-4">
                    ...
                    <div><h2 className="text-2xl font-black text-white">{selectedVerif.name}</h2><p className="text-sm text-white/30">System ID: {selectedVerif.id.toUpperCase()}</p></div>
                  </div>
                ...
                <div className="p-8 grid grid-cols-1 md:grid-cols-2 gap-8 text-sm">
                  <div className="space-y-6">
                    <div><p className="text-[10px] font-black text-white/30 uppercase tracking-widest mb-3">Organization Details</p>
                    ...
                    <div><p className="text-[10px] font-black text-white/30 uppercase tracking-widest mb-3">Uploaded Documents</p>
                      {selectedVerif.documents.map((doc: string, idx: number) => (
                        <div key={idx} className="flex items-center justify-between p-3 rounded-xl bg-white/5 border border-white/10 hover:border-white/20 transition-all cursor-pointer mb-2 group">
                  ...
                  <div className="bg-white/3 rounded-2xl p-6 border border-white/10">
                    ...
                    <textarea className="w-full h-32 bg-black/40 border border-white/10 rounded-xl p-4 text-white/60 focus:outline-none focus:border-green-500/50 transition-all text-sm resize-none" placeholder="Add verification notes..." />
                    <button className="w-full mt-4 py-3 rounded-xl border border-white/10 text-white/40 font-bold hover:bg-white/5 transition-all text-xs uppercase tracking-widest">Request More Information</button>
                  </div>
                </div>
              </motion.div>
            ) : (
              <div className="h-[400px] border-2 border-dashed border-white/5 rounded-[40px] flex flex-col items-center justify-center text-center">
                ...
                <p className="font-black text-sm text-white/20 uppercase tracking-widest">Select an organization from the left</p>
              </div>
            )}
          </div>
        </div>
  ...
  const renderSystemLogs = () => {
    ...
    return (
      <div className="space-y-6 relative z-10">
        <div className="flex flex-col md:flex-row justify-between items-start gap-4">
          <div>
            <h1 className="text-4xl font-display font-black tracking-tighter italic uppercase">System Logs</h1>
          ...
          <div className="flex gap-3 items-center">
            <div className="flex items-center gap-2 px-3 py-1.5 rounded-full bg-green-500/10 border border-green-500/20 text-green-400 text-xs font-black"><div className="w-2 h-2 rounded-full bg-green-400 animate-pulse" /> LIVE</div>
            {['All', 'INFO', 'WARN', 'ERROR'].map(f => (
              <button key={f} onClick={() => setLogFilter(f)} className={cn("px-4 py-2 rounded-xl text-xs font-black tracking-widest transition-all", logFilter === f ? (f === 'ERROR' ? 'bg-red-500 text-white' : f === 'WARN' ? 'bg-yellow-500 text-black' : 'bg-green-500 text-white') : 'bg-white/5 text-white/30 hover:text-white hover:bg-white/10')}>{f}</button>
            ))}
          </div>
        </div>
        <div className="glass-dark rounded-[40px] border border-white/5 shadow-2xl overflow-hidden">
          <div className="flex items-center gap-4 px-8 py-4 border-b border-white/5 bg-white/2">
            ...
            <span className="text-xs font-black text-white/30 uppercase tracking-widest font-mono">zerowaste-core-v7.4 // root@sys-admin</span>
            ...
            <AnimatePresence initial={false}>
              {filtered.map((log, i) => (
                <motion.div key={`${log.ts}-${i}`} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} className="flex items-start gap-4 text-xs group hover:bg-white/2 px-3 py-1.5 rounded-xl transition-colors">
                  ...
                  <span className={cn("shrink-0 w-12 font-black", log.level === 'ERROR' ? 'text-red-400' : log.level === 'WARN' ? 'text-yellow-400' : 'text-green-400')}>{log.level}</span>
  ...
  const renderEnvironment = () => {
    const Toggle = ({ value, onChange }: { value: boolean; onChange: () => void }) => (
      <button onClick={onChange} className={cn("w-14 h-7 rounded-full transition-all relative border shrink-0", value ? 'bg-green-500 border-green-400' : 'bg-white/10 border-white/10')}>
    ...
    return (
      <div className="space-y-8 relative z-10 max-w-4xl">
        <div>
          <h1 className="text-4xl font-display font-black tracking-tighter italic uppercase">Environment Config</h1>
          <p className="text-sm font-bold text-white/20 uppercase tracking-[0.4em] mt-2">System-wide feature flags and thresholds</p>
        </div>

        <div className="glass-dark rounded-[40px] border border-white/10 p-8 shadow-2xl space-y-6">
          <h3 className="text-sm font-black text-white/40 uppercase tracking-widest flex items-center gap-2"><ToggleRight className="w-4 h-4 text-green-400" /> Feature Toggles</h3>
          {[
        ...
        <div className="glass-dark rounded-[40px] border border-white/10 p-8 shadow-2xl space-y-6">
          <h3 className="text-sm font-black text-white/40 uppercase tracking-widest flex items-center gap-2"><SlidersHorizontal className="w-4 h-4 text-green-400" /> Operational Thresholds</h3>
          {[
            ...
            <div key={key} className="space-y-3 py-4 border-b border-white/5 last:border-0">
              <div className="flex justify-between items-center">
                <p className="font-black text-sm">{label}</p>
                <span className="px-3 py-1 rounded-xl bg-green-500/10 text-green-400 text-xs font-black border border-green-500/20">{(envSettings as any)[key]} {unit}</span>
              </div>
              ...
              <div className="flex justify-between text-[10px] font-black text-white/20"><span>{min} {unit}</span><span>{max} {unit}</span></div>
            </div>
          ))}
        </div>

        <button className="px-10 py-5 rounded-2xl bg-green-500 text-white font-black tracking-widest hover:bg-green-600 transition-all shadow-xl shadow-green-500/20 active:scale-95 flex items-center gap-3">
  ...
  const renderProfile = () => (
    <div className="max-w-4xl mx-auto space-y-8 relative z-10">
      <div>
        <h1 className="text-4xl font-display font-black tracking-tighter italic uppercase">Admin Profile</h1>
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="glass-dark rounded-[48px] p-10 border border-white/10 shadow-2xl">
        <div className="flex items-center gap-8 mb-10 pb-10 border-b border-white/5">
          ...
          <div>
            <h2 className="text-3xl font-display font-black italic">{adminProfile.name}</h2>
            <div className="flex flex-wrap gap-3 mt-3">
              <span className="px-3 py-1 rounded-full bg-green-500/10 text-green-400 text-[10px] font-black border border-green-500/20">ROOT ACCESS</span>
              <span className="px-3 py-1 rounded-full bg-red-500/10 text-red-400 text-[10px] font-black border border-red-500/20">{adminProfile.clearance}</span>
              <span className="px-3 py-1 rounded-full bg-white/5 text-white/40 text-[10px] font-black border border-white/10">ID: {adminProfile.id}</span>
            </div>
          </div>
        </div>
            ...
            {[
              ...
              <div key={key} className="space-y-3">
                ...
                <div className="relative group">
                  ...
                  <input type="text" value={(adminProfile as any)[key]} onChange={e => setAdminProfile({ ...adminProfile, [key]: e.target.value })} className="w-full bg-white/2 border border-white/5 rounded-[20px] py-5 pl-14 pr-5 text-sm font-black focus:outline-none focus:bg-white/5 focus:ring-2 focus:ring-green-500/20 transition-all" />
                </div>
              </div>
            ))}
          </div>
          <div className="flex gap-4 pt-4 flex-wrap">
            <button type="submit" className="px-10 py-5 rounded-2xl bg-green-500 text-white font-black tracking-widest hover:bg-green-600 transition-all shadow-xl shadow-green-500/20 active:scale-95 flex items-center gap-3">
              <Save className="w-5 h-5" /> SAVE CHANGES
            </button>
            <button type="button" onClick={() => setShowPwChange(!showPwChange)} className="px-8 py-5 rounded-2xl bg-white/5 border border-white/10 text-white/60 font-black tracking-widest hover:bg-white/10 transition-all flex items-center gap-3">
              <Lock className="w-5 h-5" /> CHANGE PASSWORD
            </button>
            <AnimatePresence>
              {profileSaved && (
                <motion.div initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0 }} className="flex items-center gap-2 text-emerald-400 font-black text-sm self-center">
        ...
        <AnimatePresence>
          {showPwChange && (
            <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} className="mt-8 pt-8 border-t border-white/5 overflow-hidden">
              <h4 className="font-black text-sm uppercase tracking-widest mb-6 flex items-center gap-2"><Lock className="w-4 h-4 text-green-400" /> Change Password</h4>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                {['Current Password', 'New Password', 'Confirm Password'].map(label => (
                  <div key={label} className="space-y-3">
                    <label className="text-[10px] font-black text-white/30 uppercase tracking-[0.3em]">{label}</label>
                    <div className="relative">
                      <input type={showPw ? 'text' : 'password'} className="w-full bg-white/2 border border-white/5 rounded-[20px] py-5 pl-5 pr-14 text-sm font-black focus:outline-none focus:bg-white/5 focus:ring-2 focus:ring-green-500/20 transition-all placeholder:text-white/10" placeholder="••••••••" />
              ...
              <button className="mt-6 px-8 py-4 rounded-2xl bg-green-500 text-white font-black text-xs tracking-widest hover:bg-green-600 transition-all active:scale-95">UPDATE PASSWORD</button>
            </motion.div>
          )}
        </AnimatePresence>
      </motion.div>
      ...
      <div className="grid grid-cols-3 gap-6">
        {[
          ...
          <div key={i} className="glass-dark rounded-[32px] p-6 border border-white/5 flex items-center gap-6">
            ...
            <div><p className="text-2xl font-display font-black">{s.value}</p><p className="text-[9px] text-white/30 font-black uppercase tracking-widest">{s.label}</p></div>
          </div>
        ))}
      </div>
    </div>
