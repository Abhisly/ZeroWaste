...
interface DashboardLayoutProps {
  ...
  title: string;
  portalType: 'restaurant' | 'ngo' | 'admin';
}
...
export default function DashboardLayout({ children, sidebarItems, title, portalType }: DashboardLayoutProps) {
  ...
  return (
    <div className="min-h-screen bg-[#050505] text-white flex font-sans overflow-hidden">
      ...
      <div className={cn(
        "fixed top-[-10%] left-[-10%] w-[50%] h-[50%] blur-[120px] rounded-full opacity-20 pointer-events-none bg-gradient-to-br",
        themeConfig.accent.replace('bg-', 'bg-')
      )} />
      ...
      <motion.aside
        ...
        className="glass-dark border-r border-white/5 flex flex-col h-screen z-50 relative shadow-2xl"
      >
        <div className="px-8 py-6 flex items-center h-28 shrink-0 border-b border-white/5">
          {isSidebarOpen ? (
            ...
            <motion.span
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
              className={`text-xl font-display font-black italic ${themeConfig.wordColor}`}
            >
              ZW
            </motion.span>
          )}
        ...
        <div className="p-4 border-t border-white/5 shrink-0">
          <button
            onClick={() => navigate('/')}
            className="flex items-center gap-4 p-4 w-full rounded-2xl text-white/40 hover:bg-red-500/10 hover:text-red-400 transition-all group font-black text-sm"
          >
      ...
      <main className="flex-1 flex flex-col h-screen overflow-hidden relative">
        {/* Top Header */}
        <header className="h-24 glass-dark border-b border-white/5 px-8 flex items-center justify-between z-40 shrink-0">
          <div className="flex items-center gap-6">
            ...
            <div>
              <h2 className="text-xl font-display font-black tracking-tight uppercase italic">{title}</h2>
          ...
          <div className="flex items-center gap-4">
              ...
              <AnimatePresence>
                {showNotifications && (
                  <motion.div
                    ...
                    className="absolute top-16 right-0 w-80 glass-dark border border-white/10 rounded-[32px] p-6 shadow-2xl z-[100] backdrop-blur-3xl bg-black/80"
                  >
                    <div className="flex justify-between items-center mb-6">
                      <h4 className="text-xs font-black tracking-widest uppercase italic">Pulse Monitor</h4>
                      ...
                      {notifications.map((n, i) => (
                        <div key={i} className="flex gap-4 p-3 rounded-2xl hover:bg-white/5 transition-colors cursor-pointer group">
                          ...
                          <div className="flex-1 min-w-0">
                            <p className="text-xs font-black truncate tracking-tight">{n.title}</p>
                            <p className="text-[10px] font-bold text-white/40 truncate mt-0.5">{n.text}</p>
                            <p className="text-[8px] font-black text-white/20 mt-1 uppercase tracking-widest">{n.time}</p>
                          </div>
                        </div>
                      ))}
                    </div>
                    <button className="w-full py-4 mt-6 rounded-2xl bg-white/5 hover:bg-white/10 text-[9px] font-black tracking-[0.2em] uppercase transition-all">
                      ARCHIVE SIGNALS
                    </button>
                  </motion.div>
                )}
            ...
            <div className="flex items-center gap-4 pl-6 ml-2 border-l border-white/5">
              <div className="text-right hidden sm:block">
                <p className="text-xs font-black tracking-tight">{portalType === 'restaurant' ? 'Amul Rest.' : portalType === 'ngo' ? 'Mission Hub' : 'System Root'}</p>
                <p className="text-[8px] font-black text-white/20 tracking-widest uppercase">NODE ACTIVE</p>
              </div>
